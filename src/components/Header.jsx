...
// Audio file for notification sound (assumed to be in public/sounds/)
...
const Header = ({ toggleSidebar, toggleSidebarCollapse, isSidebarOpen }) => {
  ...
  useEffect(() => {
    ...
    const derivedId =
      sessionStorage.getItem("adminId") || getIdFromToken(token);
  ...
  const fetchNotifications = async () => {
    try {
      const response = await fetch(
        "https://backend-6bli.onrender.com/admin/notifications",
        {
          headers: {
    ...
    <>
      <header className="flex justify-between items-center bg-white shadow-md px-6 py-3 sticky top-0 z-50">
        <div className="flex items-center gap-4">
          <Bars3Icon
            className="h-6 w-6 text-gray-800 cursor-pointer hover:text-[#ffffff] transition-colors duration-200 md:hidden"
